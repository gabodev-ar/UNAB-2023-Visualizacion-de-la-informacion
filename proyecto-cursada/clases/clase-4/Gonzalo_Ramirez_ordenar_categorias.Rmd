...
```{r}
ggplot(diamantes, aes(fct_relevel(color, "J", "I", "D", "H", "F", "E", "G"))) +
  geom_bar()
```

Ahora probá ordenar las mismas categorías pero en orden creciente de frecuencia.

##COMENTARIO: Como en el ejercicio anterior, sin saber, ya había ordenado las categorías
##por su frecuencia, en este ejercicio decidí con diplyer modifiqué el nombre de la columna.

```{r}
diamantes %>% 
    mutate(color = fct_relevel(color, "J", "I", "D", "H", "F", "E", "G")) %>% 
    ggplot(aes(color)) +
    geom_bar()
```
